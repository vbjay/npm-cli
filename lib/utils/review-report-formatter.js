...
const formatBuildIndicatorSection = (indicators, lines) => {
...
const formatMarkdown = (packages) => {
  ...
  for (const pkg of packages) {
    ...
    if (pkg.buildInfo && pkg.buildInfo.length > 0) {
      formatBuildIndicatorSection(pkg.buildInfo, lines)
    }
    ...
    const focus = buildAllReviewFocus(signals, pkg.buildInfo)
    if (focus.length > 0) {
...
const formatJson = (packages) => {
  const enriched = packages.map((pkg) => {
    ...
    return {
      ...
      suggestedReviewFocus: buildAllReviewFocus(signals, pkg.buildInfo),
