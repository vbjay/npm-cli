...
// classifications are surfaced; 'reference' URLs are excluded to avoid
// misleading the reviewer into thinking a homepage link is a download target.
const enrichBuildInfo = (buildInfo, referencedFiles) => {
  ...
  const downloadUrls = [...new Set(
    files
      ...
      .flatMap((f) => f.urls)
      .filter((u) => (u.classification || classifyUrl(u.url ?? u)) !== 'reference')
      .map((u) => u.url ?? u)
  )]
