...
// Classify a URL as 'download', 'reference', or 'unknown'.
//
// 'download'  — high confidence this URL is a binary/archive download target:
//   archive/binary file extension, known download path pattern, or a known
//   download-only domain (npm registry, GitHub raw/release assets, etc.)
//
// 'reference' — high confidence this is a homepage, funding, docs, or license
//   URL, not a download target.  Showing it under "binary download" would
//   mislead the reviewer.
//
// 'unknown'   — insufficient signal; treat as a generic external URL reference.
...
// Only URLs classified as 'download' or 'unknown' are included — confirmed
// 'reference' URLs (homepages, funding, docs) are excluded to avoid misleading
// the reviewer into thinking a homepage link is a binary download target.
const enrichBuildInfo = (buildInfo, referencedFiles) => {
  ...
  const downloadUrls = [...new Set(
    files
      ...
      .flatMap((f) => f.urls)
      .filter((url) => classifyUrl(url) !== 'reference')
  )]
