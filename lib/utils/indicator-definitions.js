...
const INDICATOR_REGISTRY = {
  ...
  //
  // Also triggered by the 'makes-executable' signal (chmod+x / chmod 755 etc.)
  // on a lifecycle-scanned file.  Packages like @icp-sdk/ic-wasm pre-bundle
  // a platform binary inside the npm tarball and make it executable at install
  // time — no network fetch occurs, but the binary still warrants review.
  // The 'makes-executable' signal is the discriminator: a telemetry POST or
  // a JSON config download never needs to flip the execute bit.
  // -------------------------------------------------------------------------
  'binary-downloader': {
    ...
    detect: {
      ...
      // chmod+x on a lifecycle-scanned file is strong evidence that a fetched or
      // bundled binary is meant to be run.  Ordinary data downloads never need
      // the execute bit, so this signal narrows the false-positive rate relative
      // to a plain 'external-url' trigger.
  ...
  // examine the URLs and surrounding code before drawing conclusions.  If the
  // 'makes-executable' signal is also present the 'binary-downloader' indicator
  // fires instead, which is the higher-confidence classification.
  'source-downloader': {
    label: 'External URL fetch',
    detect: {
      commandPatterns: [
