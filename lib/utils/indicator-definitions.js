...
const INDICATOR_REGISTRY = {
  ...
  'binding.gyp': {
    ...
    detect: {
      commandPatterns: [
        ...
        /\bnpm\s+rebuild\b/,
        /\bpnpm\s+rebuild\b/,
        // Packages with a proprietary install/<script> that wraps prebuild-or-gyp
        // logic — sharp is the canonical example (node install/check downloads
        // a platform-specific prebuilt or falls back to node-gyp build).
        /\bnode\s+install\//,
        // Packages that run their own CLI entry point to orchestrate a build
        // (e.g. npm's prepack: "node . run build -w docs" where "node ." invokes
        // the package's main entry as the CLI and "run" is a subcommand).
        /\bnode\s+\.\s+run\b/,
      ],
  ...
  'Cargo.toml': {
    ...
    detect: {
      commandPatterns: [
        /\bnapi\s+build\b/,
        /\bnapi\s+cross\b/,           // cross-compile for multiple targets
        /\bnapi\s+artifacts\b/,        // collect prebuilt binaries (napi-rs CI)
        /\bnapi\s+prepublish-only\b/,  // napi-rs publish flow
        /\bneon\s+build\b/,
        /\bcargo\s+build\b/,
        /\bwasm-pack\b/,
        /\bcargo\s+install\b/,    // tools installed via cargo install run at install time
      ],
  ...
  // -------------------------------------------------------------------------
  // Meson — a modern C/C++ build system, often paired with Ninja as the
  // backend, replacing Autoconf/Make in many newer native packages.
  // -------------------------------------------------------------------------
  'meson.build': {
    label: 'Meson build descriptor',
    detect: {
      commandPatterns: [
        /\bmeson\s+(?:setup|compile|build|install|configure)\b/,
        /\bninja\s+(?:-C\b|-j\b|all\b|install\b|build\b)/,
      ],
      triggeredByNativeBuildSignal: false,
    },
    signals: {
    ...
    scanner: {
      type: 'generic',
      steps: [
        // executable() / library() — the actual compiled targets.
        {
          type: 'regex-all',
          pattern: /(?:executable|library|shared_library|static_library|shared_module)\s*\(\s*['"]([^'"]+)['"]/g,
          group: 1, label: 'Compiled targets'
        },
        // dependency() — system/pkg-config libraries required at build time.
        {
          type: 'regex-all',
          pattern: /\bdependency\s*\(\s*['"]([^'"]+)['"]/g,
          group: 1, label: 'Library dependencies'
        },
        // find_program() — external build tools required (compilers, code generators).
        {
          type: 'regex-all',
          pattern: /\bfind_program\s*\(\s*['"]([^'"]+)['"]/g,
          group: 1, label: 'External programs required'
        },
  ...
  'runtime-installer': {
    ...
    detect: {
      commandPatterns: [
        ...
        // Direct npm/yarn/pnpm/bun install commands inside lifecycle scripts or their
        // npm-run delegations (e.g. "build:web-ui":"cd sub && npm install && npm run build").
        // extractLifecycleScripts() follows npm run chains, so these patterns fire even
        // when the install call is in a non-hook script reached via delegation.
        /\bnpm\s+install\b/,
        /\bnpm\s+ci\b/,
        /\byarn\s+install\b/,
        /\byarn\s+add\b/,
  ...
  // -------------------------------------------------------------------------
  // Native build command (virtual) — triggered when the deep scanner detects
  // a native build invocation (node-gyp, cmake, cargo, meson, ninja, grunt,
  // gulp, etc.) inside a lifecycle script file but no build descriptor file
  // (binding.gyp, CMakeLists.txt, Cargo.toml, meson.build, Gruntfile.js, …)
  // was found on disk.
  //
  // This covers packages that orchestrate native builds without shipping the
  // build descriptor alongside them — e.g. tool installers that compile from
  // source using their own configure/bootstrap scripts, or library-builder
  // helpers that spawn a task runner in the consuming project's directory.
  //
  // The file-backed indicators (binding.gyp, Cargo.toml, etc.) already carry
  // triggeredByNativeBuildSignal: true, so packages that HAVE a build
  // descriptor will be handled there.  This virtual entry fills the gap for
  // packages that invoke native tooling but ship no descriptor.
  //
  // Supply-chain risk: a lifecycle script that runs native build tooling
  // outside a standard build descriptor is worth manual inspection — the
  // reviewer cannot simply read the descriptor to understand what is being
  // compiled or where the output goes.  They must trace the actual script
  // execution.
  // -------------------------------------------------------------------------
  'native-build-command': {
    label: 'Native build command in lifecycle script (no descriptor file)',
    detect: {
      commandPatterns: [],
      triggeredByNativeBuildSignal: true,
    },
  ...
  // -------------------------------------------------------------------------
  // Obfuscated install script — virtual indicator triggered when the
  // script-risk-scanner finds Base64/hex-decode+eval, jsfuck, or similar
  // obfuscation patterns inside a lifecycle file.  Obfuscation at install
  // time is a strong signal that the script is hiding its intent from static
  // reviewers and automated supply-chain tools.
  //
  // Supply-chain risk: legitimate packages have no reason to obfuscate their
  // postinstall logic.  Reviewers should decode and manually inspect the
  // obfuscated sections before approving.  Common examples include minified
  // downloader scripts that encode platform selection and download URLs to
  // defeat pattern matching (e.g. electron's install.js).
  // -------------------------------------------------------------------------
  'obfuscated-install-script': {
    label: 'Obfuscated install script',
    detect: {
      commandPatterns: [],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      ...
      triggeredByExternalUrlSignal: false,
      triggeredByMakesExecutableSignal: false,
      triggeredByObfuscationPatternSignal: true,
    },
    signals: {
      onFound: ['obfuscation-pattern'],
      onWarning: [],
    },
    scanner: { type: 'none' },
  },
  ...
  // -------------------------------------------------------------------------
  // Dynamic-require loader — virtual indicator triggered when the
  // script-risk-scanner detects a require() call with a computed (non-literal)
  // argument in a lifecycle file.  The module actually loaded cannot be
  // determined by static analysis — it depends on runtime state such as
  // process.env, process.platform, or an external config file.
  //
  // Supply-chain risk: dynamic require can load different modules per
  // environment, bypassing static review.  Reviewers should trace all values
  // that can flow into the require() argument and verify each possible path.
  // -------------------------------------------------------------------------
  'dynamic-require-loader': {
    label: 'Dynamic-require loader',
    detect: {
      commandPatterns: [],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      ...
      triggeredByExternalUrlSignal: false,
      triggeredByMakesExecutableSignal: false,
      triggeredByObfuscationPatternSignal: false,
      triggeredByDynamicRequireSignal: true,
    },
    signals: {
      onFound: ['dynamic-require'],
      onWarning: [],
    },
    scanner: { type: 'none' },
  },

  // Root-level build.gradle — React Native and Capacitor packages sometimes
  // place the Gradle descriptor at the package root rather than under android/.
  // Same signals as android/build.gradle; narrower commandPatterns to avoid
  // false-positives on pure-Java projects that have no npm lifecycle scripts.
  'build.gradle': {
    label: 'Gradle build script (root-level)',
    detect: {
      commandPatterns: [
        /\bgradlew?\b/,
        /\breact-native\s+build-android\b/,
        /\bcap\s+(?:build|sync|open|run)\b/,
        /\bexpo-module\b/,
      ],
      triggeredByNativeBuildSignal: false,
    },
    ...
    scanner: {
      ...
      steps: [
        ...
        { type: 'glob', pattern: '**/*.{c,cpp,cc,h}', label: 'C/C++ source files', maxDisplay: 20 },
      ],
    },
  },
  ...
  'bower.json': {
    ...
    scanner: {
      ...
      steps: [
        ...
        {
          type: 'regex-all',
          pattern: /"dependencies"\s*:\s*\{([^}]+)\}/gs, group: 1,
  ...
  'brunch-config.js': {
    ...
    scanner: {
      ...
      steps: [
        ...
        {
          type: 'regex-all',
          pattern: /plugins\s*:\s*\{([^}]+)\}/gs, group: 1,
  ...
  'Taskfile.yml': {
    ...
    detect: {
      commandPatterns: [
        /\btask\s+(?:install|build|compile|all|native|setup|rebuild|clean|default|generate|dist|release|run|package|bundle|make)\b/,
      ],
      triggeredByNativeBuildSignal: false,
    },
    signals: {
  ...
  // Alias entries for Gulpfile and Gruntfile variants — same signals and scanner
  // as the primary entries above; the registry key IS the filename checked on disk
  // so each variant needs its own entry.
  'gulpfile.ts': {
    label: 'Gulp task runner script (TypeScript)',
    detect: { commandPatterns: [/\bgulp\b/, /\bgulp-cli\b/], triggeredByNativeBuildSignal: false },
    signals: { onFound: ['make-build'], onWarning: [] },
    scanner: {
      type: 'generic',
      steps: [
        { type: 'regex-all', pattern: /gulp\.task\s*\(\s*['"]([^'"]+)['"]/g, group: 1, label: 'Registered Gulp tasks (v3 API)' },
        { type: 'regex-all', pattern: /exports\.([A-Za-z_$][\w$]*)\s*=/g, group: 1, label: 'Exported Gulp tasks (v4 API)' },
        { type: 'regex-all', pattern: /\b(node-gyp|cmake|cmake-js|make|nmake|cargo|curl|wget|python[23]?)\b/g, group: 1, label: 'External tools invoked in gulpfile' },
        { type: 'glob', pattern: 'gulpfile.{js,ts,mjs,cjs,coffee}', label: 'Gulpfile variants', maxDisplay: 5 },
      ],
    },
  },
  'gulpfile.mjs': {
    label: 'Gulp task runner script (ESM)',
    detect: { commandPatterns: [/\bgulp\b/, /\bgulp-cli\b/], triggeredByNativeBuildSignal: false },
    signals: { onFound: ['make-build'], onWarning: [] },
    scanner: {
      type: 'generic',
      steps: [
        { type: 'regex-all', pattern: /export\s+(?:async\s+)?function\s+(\w+)/g, group: 1, label: 'Exported Gulp tasks (ESM named exports)' },
        { type: 'regex-all', pattern: /\b(node-gyp|cmake|cmake-js|make|nmake|cargo|curl|wget|python[23]?)\b/g, group: 1, label: 'External tools invoked in gulpfile' },
        { type: 'glob', pattern: 'gulpfile.{js,ts,mjs,cjs,coffee}', label: 'Gulpfile variants', maxDisplay: 5 },
      ],
    },
  ...
  'Gruntfile.coffee': {
    label: 'Grunt build script (CoffeeScript)',
    detect: { commandPatterns: [/\bgrunt\b/, /\bgrunt-cli\b/], triggeredByNativeBuildSignal: false },
    signals: { onFound: ['make-build'], onWarning: [] },
    scanner: {
      type: 'generic',
      steps: [
        { type: 'regex-all', pattern: /grunt\.registerTask\s*['"]([^'"]+)['"]/g, group: 1, label: 'Registered Grunt tasks' },
        { type: 'regex-all', pattern: /\b(node-gyp|cmake|cmake-js|make|nmake|curl|wget|python[23]?)\b/g, group: 1, label: 'External tools invoked in Gruntfile' },
        { type: 'glob', pattern: 'Gruntfile.{js,coffee}', label: 'Gruntfile variants', maxDisplay: 5 },
      ],
    },
  },
  'Jakefile.js': {
    label: 'Jake task runner script (.js variant)',
    detect: { commandPatterns: [/\bjake\b/], triggeredByNativeBuildSignal: false },
    signals: { onFound: ['make-build'], onWarning: [] },
    scanner: {
      type: 'generic',
      steps: [
        { type: 'regex-all', pattern: /(?:^|\s)(?:task|file|directory)\s*\(\s*['"]([^'"]+)['"]/gm, group: 1, label: 'Jake tasks' },
        { type: 'regex-all', pattern: /\b(node-gyp|cmake|make|cargo|cc|curl|wget|python[23]?)\b/g, group: 1, label: 'External tools invoked in Jakefile' },
        { type: 'glob', pattern: 'Jakefile{,.js,.coffee}', label: 'Jakefile variants', maxDisplay: 5 },
      ],
    },
  },
  // Taskfile.yaml — alternate extension for go-task (same as Taskfile.yml)
  'Taskfile.yaml': {
    label: 'Taskfile (go-task) task runner (.yaml variant)',
    detect: { commandPatterns: [/\btask\s+(?:install|build|compile|all|native|setup|rebuild|clean|default|generate|dist|release|run|package|bundle|make)\b/], triggeredByNativeBuildSignal: false },
    signals: { onFound: ['make-build'], onWarning: [] },
    scanner: {
      type: 'generic',
      steps: [
        { type: 'regex-all', pattern: /^\s{2}([A-Za-z_-][\w-]*):/gm, group: 1, label: 'Taskfile tasks' },
        { type: 'regex-all', pattern: /\b(node-gyp|cmake|make|cargo|cc|curl|wget|python[23]?)\b/g, group: 1, label: 'External tools invoked in Taskfile' },
        { type: 'glob', pattern: 'Taskfile.{yml,yaml}', label: 'Taskfile variants', maxDisplay: 5 },
      ],
    },
  },
  ...
  'Justfile': {
    ...
    detect: {
      commandPatterns: [
        ...
        /\bjust\s+(?:install|build|compile|all|native|setup|run|rebuild|generate|dist|release|package|bundle|make|link|fmt|format|check|default)\b/,
      ],
      triggeredByNativeBuildSignal: false,
    },
    signals: {
  ...
  // -------------------------------------------------------------------------
  // TypeScript compilation — virtual indicator (no file on disk).
  // Fires when a lifecycle script (or a script it delegates to via `npm run`)
  // invokes the TypeScript compiler or a TypeScript-based bundler.
  //
  // Risk level: low — TypeScript compilation is a standard build step, not
  // a security concern.  Categorising these packages prevents them from
  // appearing in the uncategorised list and lets reviewers focus on higher-
  // risk indicators.
  // -------------------------------------------------------------------------
  'typescript-compiler': {
    ...
    detect: {
      commandPatterns: [
        /\btsc\b/,          // TypeScript compiler CLI  (tsc, tsc -b, tsc -p tsconfig.json)
        /\btsup\b/,         // tsup — fast TypeScript bundler built on esbuild
        /\bts-node\b/,      // ts-node — TypeScript execution environment
        /\bts-library\b/,   // ts-library task name used by monorepo build tools (e.g. dev-tools)
        /\bbun\b/,          // bun — TypeScript-native runtime/bundler (bun run scripts/build.ts etc.)
      ],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      triggeredByRuntimeInstallerSignal: false,
  ...
  // -------------------------------------------------------------------------
  // Linter / formatter — virtual indicator (no file on disk).
  // Fires when a lifecycle script (or a script it delegates to via `npm run`)
  // invokes a code-quality or formatting tool.
  //
  // Risk level: low — linting is a standard quality-gate step.  Categorising
  // these packages reduces the uncategorised count without inflating risk.
  // -------------------------------------------------------------------------
  'lint-runner': {
    label: 'Linter / formatter',
    detect: {
      commandPatterns: [
        /\beslint\b/,    // ESLint — most popular JavaScript/TypeScript linter
        /\btslint\b/,    // TSLint — deprecated TypeScript linter
        /\bprettier\b/,  // Prettier — opinionated code formatter
        /\blint\b/,      // generic: npm run lint, lint-staged, etc.
      ],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      triggeredByRuntimeInstallerSignal: false,
  ...
  // -------------------------------------------------------------------------
  // Platform-specific binary selector — virtual indicator (no file on disk).
  // Fires when a lifecycle script (or a file it delegates to via `node <path>`)
  // inspects process.platform / os.platform() / os.arch() to select, cache, or
  // activate a platform-specific binary from an optional-dependency subpackage.
  //
  // Common pattern: a postinstall.mjs / setup-cache.js that finds the correct
  // prebuilt binary (e.g. @pkg/pkg-darwin-arm64), caches its resolved path,
  // copies it to the npm bin directory, or simply verifies it exists.  The
  // script runs different code paths depending on the host OS and architecture.
  //
  // Risk level: medium — reviewers should verify what each platform branch
  // does: which binary is selected, where it comes from, whether integrity is
  // checked, and that no unsigned or unexpected executable is activated.
  // -------------------------------------------------------------------------
  'platform-binary-selector': {
    label: 'Platform-specific binary selector or activator',
    detect: {
      commandPatterns: [],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      ...
      triggeredByExternalUrlSignal: false,
      triggeredByMakesExecutableSignal: false,
      triggeredByPlatformSpecificScriptSignal: true,
    },
    signals: {
      onFound: ['platform-specific-install'],
      onWarning: [],
    },
    scanner: { type: 'none' },
  },
  ...
  // -------------------------------------------------------------------------
  // Git hook manager — virtual indicator (no file on disk).
  // Fires when a lifecycle script invokes a git-hook manager such as husky,
  // lefthook, or simple-git-hooks.  The deep scanner also follows into the
  // .husky/ and .lefthook/ hook script directories so that any build-relevant
  // or risky commands inside those scripts are surfaced as additional signals.
  //
  // Risk level: low — installing git hooks is a standard developer-tooling
  // step.  Categorising these packages prevents them from appearing in the
  // uncategorised list and lets reviewers focus on higher-risk indicators.
  // -------------------------------------------------------------------------
  'git-hook-runner': {
    label: 'Git hook manager',
    detect: {
      commandPatterns: [
        /\bhusky\b/,            // husky — most popular git hook manager
        /\blefthook\b/,         // lefthook — fast cross-platform hook manager
        /\bsimple-git-hooks\b/, // simple-git-hooks — minimal hook manager
        /\bpinst\b/,            // pinst — disables husky hooks in CI (often paired)
      ],
      triggeredByNativeBuildSignal: false,
      triggeredByBinaryDownloadSignal: false,
      triggeredByRuntimeInstallerSignal: false,
...
const NATIVE_BUILD_COMMAND_PATTERN =
  /(?<!\/)\bnode-gyp\b(?!\s*\|)|\bnode-gyp-build\b|\bnode-gyp-build-optional-packages\b|binding\.gyp|\bnode-pre-gyp\b|\bprebuild-install\b|\bprebuildify\b|\bcmake-js\b|\bnapi\s+build\b|\bneon\s+build\b|\belectron-rebuild\b|\belectron-build-env\b|\bprebuild\b|\.\/configure\b|\bgrunt\b|\bgulp\b|\bbrunch\b|\bmeson\s+(?:setup|compile|build)\b|\bninja\b/
...
const SIGNAL_DESCRIPTIONS = {
  ...
  'typescript-build': 'Compiles TypeScript source to JavaScript at prepare/install time (tsc, tsup, ts-node)',
  'lint-check': 'Runs code quality and style checks at prepare/install time (ESLint, Prettier, etc.)',
  'git-hook-setup': 'Installs or configures git hooks (husky, lefthook, simple-git-hooks) — hook scripts are scanned for build or network signals',
  'platform-specific-install': 'Lifecycle script inspects process.platform / os.platform() to select or activate a platform-specific binary — different code paths run per OS/architecture',
}
