...
const INDICATOR_REGISTRY = {
  ...
  'runtime-installer': {
    ...
    detect: {
      commandPatterns: [],   // matched by signal, not by command text
      triggeredByNativeBuildSignal: false,
...
const NATIVE_BUILD_COMMAND_PATTERN =
  /(?<!\/)\bnode-gyp\b(?!\s*\|)|\bnode-gyp-build\b|\bnode-gyp-build-optional-packages\b|binding\.gyp|\bnode-pre-gyp\b|\bprebuild-install\b|\bprebuildify\b|\bcmake-js\b|\bnapi\s+build\b|\bneon\s+build\b|\belectron-rebuild\b|\belectron-build-env\b|\bprebuild\b|\.\/configure\b/
