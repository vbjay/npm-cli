...
const getDepPaths = require('./dep-path-walker.js')
const classifyScriptChange = require('./script-change-classifier.js')
const scanPackageScripts = require('./script-risk-scanner.js')
const { hasBuildHint, scanBuildIndicatorsForPackage } = require('./indicator-scanner.js')
const formatReviewReport = require('./review-report-formatter.js')
...
class AllowScriptsCmd extends BaseCommand {
  static params = ['all', 'allow-scripts-pending', 'allow-scripts-pin', 'allow-scripts-report-format', 'json']
  ...
  async runMode (mode, args) {
    ...
    const reviewFormat = this.npm.config.get('allow-scripts-report-format')
    const reviewFormatWhere = this.npm.config.find('allow-scripts-report-format')
    // Only raise a usage error when the option is supplied on the command line or
    // via an environment variable.  Values from .npmrc / project / user / global
    // config files are silently ignored when not in list mode so that setting a
    // preferred format in .npmrc does not break normal approve/deny flows.
    const reviewFormatExplicit = reviewFormatWhere === 'cli' || reviewFormatWhere === 'env'
    if (reviewFormatExplicit && !list) {
      throw this.usageError(
        '`--allow-scripts-report-format` requires `--allow-scripts-pending` or `npm install-scripts ls`.'
      )
    }
    ...
    if (list) {
      // --json selects the machine-readable review report; reviewFormat
      // (default: 'markdown') selects the human-readable one.
      // Explicitly passing --allow-scripts-report-format=null opts out of
      // both and falls back to the legacy compact text listing.
      const effectiveFormat = reviewFormat === null
        ? null
        : (this.npm.flatOptions.json ? 'json' : reviewFormat)
      if (effectiveFormat) {
        return this.runReviewReport(unreviewed, effectiveFormat, arb, policy || {})
      }
  ...
  // Generate a structured review report for pending lifecycle scripts.
  // `allowScripts` is the current policy object (may be empty `{}`).
  async runReviewReport (unreviewed, format, arb, allowScripts) {
    const packages = []
    ...
    formatReviewReport(packages, format)
  }
  ...
  findNodesForArgs (args, arb) {
    ...
    for (const arg of args) {
      ...
      for (const node of arb.actualTree.inventory.values()) {
        if (node.isProjectRoot || node.isWorkspace || node.inDepBundle) {
          continue
        }
        const { name, version } = trustedDisplay(node)
  ...
  groupByPackage (nodes) {
    ...
    for (const node of nodes) {
      ...
      /* istanbul ignore if: callers prefilter via inDepBundle and trustedDisplay so untrusted nodes don't reach here */
      if (!key) {
        log.warn(
          this.logTitle,
