...
const { hasNativeBuildHint, scanNativeBuildIndicatorsForPackage } = require('./indicator-scanner.js')
...
class AllowScriptsCmd extends BaseCommand {
  ...
  async runReviewReport (unreviewed, format, arb, allowScripts) {
    ...
    for (const { node, scripts } of unreviewed) {
      ...
      const nativeBuildInfo = node.path && hasNativeBuildHint(scripts, referencedFiles)
        ? await scanNativeBuildIndicatorsForPackage(node.path, scripts, referencedFiles)
        : null
