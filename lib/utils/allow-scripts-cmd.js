...
class AllowScriptsCmd extends BaseCommand {
  ...
  async runMode (mode, args) {
    ...
    // config files are silently ignored when --allow-scripts-pending is absent so
    // that setting a preferred format in .npmrc does not break normal approve/deny
    // flows.
    const reviewFormatExplicit = reviewFormatWhere === 'cli' || reviewFormatWhere === 'env'
    if (reviewFormatExplicit && !pending) {
      throw this.usageError(
        '`--allow-scripts-report-format` requires `--allow-scripts-pending`.'
      )
    }
