...
const hasBuildHint = (scripts, referencedFiles, registry = INDICATOR_REGISTRY, deps = []) => {
  ...
  // Signal-based gating — any of these in a scanned file warrants full categorisation.
  const BUILD_HINT_SIGNALS = new Set([
    'native-build', 'binary-download', 'runtime-installer', 'external-url',
    'makes-executable', 'obfuscation-pattern', 'dynamic-require',
    'wasm-load',                  // pre-compiled WASM payload — machine code runs with JS privileges
    'shell-network-fetch',        // curl / wget fetching remote content at install time
    'uses-eval',                  // eval() / new Function() — arbitrary code execution
    'base64-decode-exec',         // base64 decoding, frequently used to hide payloads
    'jsfuck-obfuscation',         // JSFuck / extreme character-code obfuscation
    'platform-specific-script',   // process.platform / os.platform() branch — platform-binary-selector
  ])
  for (const { signals } of referencedFiles) {
    if (Array.isArray(signals) && signals.some(s => BUILD_HINT_SIGNALS.has(s))) return true
  }
...
const detectClues = async (packageDir, scripts, referencedFiles, registry) => {
  ...
  const hasObfuscationPatternSig = referencedFiles.some(
    (f) => Array.isArray(f.signals) && f.signals.includes('obfuscation-pattern')
  )
  const hasDynamicRequireSig = referencedFiles.some(
    (f) => Array.isArray(f.signals) && f.signals.includes('dynamic-require')
  )
  const hasPlatformSpecificScriptSig = referencedFiles.some(
    (f) => Array.isArray(f.signals) && f.signals.includes('platform-specific-script')
  )
  ...
  for (const [indicatorFile, def] of Object.entries(registry)) {
    ...
    if (cmdMatch) {
      ...
      (hasObfuscationPatternSig && def.detect.triggeredByObfuscationPatternSignal) ||
      (hasDynamicRequireSig && def.detect.triggeredByDynamicRequireSignal) ||
      (hasPlatformSpecificScriptSig && def.detect.triggeredByPlatformSpecificScriptSignal)
    ) {
  ...
  for (const step of steps) {
    if (step.type === 'regex') {
      ...
      if (step.presence) {
        ...
        // Always force 'g' — exec() loop requires it to advance lastIndex;
        // without it the loop spins forever on the same first match.
        const flags = step.pattern.flags.includes('g') ? step.pattern.flags : step.pattern.flags + 'g'
        const re = new RegExp(step.pattern.source, flags)
        const seen = new Set()
        const items = []
        let m
