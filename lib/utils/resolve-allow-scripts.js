...
const validatePolicy = (policy, sourceLabel) => {
  ...
  for (const [key, value] of Object.entries(policy)) {
    ...
    if (parsed.type === 'git') {
      ...
      // identity guarantees: a 1-char prefix matches ~6 % of all commits, so
      // an attacker who controls a commit whose SHA starts with that character
      // would pass the check.  Require at least 7 chars (Git's own minimum for
      // unambiguous short SHAs) or drop the entry with a warning so the
      // reviewer is prompted to use a longer, safer pin.
      const committish = parsed.gitCommittish || parsed.hosted?.committish
      if (committish && /^[0-9a-fA-F]+$/.test(committish) && committish.length < 7) {
        log.warn(
          'allow-scripts',
          `${sourceLabel}: ignoring "${key}" — git committish "${committish}" is too short ` +
          '(minimum 7 hex characters); use a longer SHA prefix or a full 40-char SHA'
        )
        continue
      }
    }
