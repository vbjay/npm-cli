...
const SIGNAL_PATTERNS = [
  ...
  ['external-url',
    /https?:\/\/[a-zA-Z0-9]/],
  ['native-build',
    // node-gyp and variants are the classic C/C++ addon build pipeline.
    // node-pre-gyp downloads a pre-built binary or falls back to compilation.
    // prebuild-install / prebuildify download prebuilt binaries from GitHub.
    // cmake-js builds native addons with CMake instead of GYP.
    // napi build (napi-rs CLI) and neon build (Rust/Neon) compile Rust-based addons.
    /\bnode-gyp\b|binding\.gyp|\bnode-pre-gyp\b|\bprebuild-install\b|\bprebuildify\b|\bcmake-js\b|\bnapi\s+build\b|\bneon\s+build\b/],
  ['shell-network-fetch',
