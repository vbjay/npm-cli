...
const MAX_DEPTH = 3               // Maximum recursion depth for local requires
...
const SIGNAL_PATTERNS = [
  ...
  ['references-credential-env-var',
    /process\.env\.(?:npm_token|github_token|token|secret|password|api_key|access_key|private_key|auth(?:orization)?|credential|aws_|ci_token)/i],
  ['network-access',
    // Matches bare and node:-prefixed built-in HTTP(S) modules as well as common HTTP libraries.
    /\brequire\s*\(\s*['"](?:node:)?(?:https?|node-fetch|axios|got|superagent|request)['"]\s*\)|\bfetch\s*\(|https?\.(?:get|request)\s*\(/i],
  ['uses-net-socket',
  ...
  ['writes-outside-package',
    // Heuristic: path manipulation using "../" or process.cwd()
    /(['"`])\.\.\/.*\1|process\.cwd\s*\(\)/],
...
const LOCAL_REQUIRE_RE = /\brequire\s*\(\s*(['"`])(\.{1,2}\/[^'"`\s\n)]+)\1\s*\)/g
const LOCAL_IMPORT_FROM_RE = /\bfrom\s+(['"`])(\.{1,2}\/[^'"`\s\n]+)\1/g
const LOCAL_IMPORT_BARE_RE = /\bimport\s+(['"`])(\.{1,2}\/[^'"`\s\n]+)\1/g
// Dynamic import() calls with a literal local specifier: import('./helper.js')
const LOCAL_DYNAMIC_IMPORT_RE = /\bimport\s*\(\s*(['"`])(\.{1,2}\/[^'"`\s\n)]+)\1\s*\)/g
// Shell source directives: `source ./file.sh` or `. ./file.sh`
// Matches at start of line or after a shell operator (; & | newline) to reduce false positives.
const SHELL_SOURCE_RE = /(?:^|[;\n&|])\s*(?:source|\.)\s+(['"`]?)(\.{1,2}\/[^\s'"`\n;]+)\1/gm

const findLocalRefs = (content) => {
  const refs = new Set()
  for (const re of [LOCAL_REQUIRE_RE, LOCAL_IMPORT_FROM_RE, LOCAL_IMPORT_BARE_RE, LOCAL_DYNAMIC_IMPORT_RE]) {
    re.lastIndex = 0
    let match
...
// Best-effort split of a lifecycle command string on shell control operators
// (&&, &, ||, ;, |).  Operators that appear inside quoted strings will also be
// split; this is acceptable given that lifecycle scripts rarely quote these
// operators and the scanner is explicitly best-effort.
...
const parseSingleCommand = (cmd, packageDir) => {
  const trimmed = cmd.trim()
  if (!trimmed) {
    return []
  }

  // Split only on unquoted whitespace (simple split is fine for common cases)
  ...
  if (interpreter === 'node' || interpreter === 'nodejs' ||
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      // Join the remaining tokens (the inline code may have been split on
      // spaces), strip a single layer of outer quotes, and scan for local refs.
      if (part === '-e' || part === '--eval' || part === '-p' || part === '--print') {
        /* istanbul ignore else */
        if (i + 1 < parts.length) {
          const raw = parts.slice(i + 1).join(' ')
          const code = raw.replace(/^(['"])([\s\S]*)\1$/, '$2').trim()
      ...
      // Non-simple bare names (special chars) are unlikely to be local
      // scripts – skip them.
      if (!part.includes('/') && !/^[a-zA-Z0-9-]+$/.test(part)) {
        break
      }
      // Bare module name that might be a local script (e.g. `node install`)
  ...
  if (['bash', 'sh', 'zsh', 'dash'].includes(interpreter)) {
    for (let i = 1; i < parts.length; i++) {
      ...
      if (p === '-c') {
        if (i + 1 < parts.length) {
          // Reconstruct the inline command from remaining parts and strip any
          // outer single or double quotes that shell quoting would add.
          const raw = parts.slice(i + 1).join(' ')
          const stripped = raw.replace(/^(['"])([\s\S]*)\1$/, '$2').trim()
          return stripped ? parseCommandFile(stripped, packageDir) : []
        }
        return []
      }
      if (!p.startsWith('-')) {
  ...
  if (interpreter === 'env') {
    ...
    if (start < parts.length) {
      return parseSingleCommand(parts.slice(start).join(' '), packageDir)
    }
    return []
  }
...
// parsed independently and the results are merged into a deduplicated list.
// Returns Array<{ filePath: string, type: 'js'|'shell'|'other' }>
const parseCommandFile = (cmd, packageDir) => {
  ...
  const subCommands = trimmed.split(SHELL_OPS_RE)
  const seen = new Set()
  const results = []
...
const scanFile = async (filePath, reason, packageDir, depth, scanned, results, maxDepth) => {
      ...
      for (const ext of ['.js', '.mjs', '.cjs']) {
        ...
        try {
          ...
          if (st.isFile()) {
            ...
            await scanFile(candidate, reason, packageDir, depth, scanned, results, maxDepth)
  ...
  for (const ref of localRefs) {
    ...
    if (resolved) {
      references.push(path.relative(packageDir, resolved))
    }
  }
  ...
  if (depth < maxDepth) {
    for (const ref of localRefs) {
      ...
      if (resolved) {
        await scanFile(
          resolved,
          `required by ./${path.relative(packageDir, filePath)}`,
          packageDir,
          depth + 1,
          scanned,
          results,
...
const scanPackageScripts = async (packageDir, scripts) => {
  ...
  for (const [event, cmd] of Object.entries(scripts)) {
    ...
    for (const { filePath } of files) {
      ...
      await scanFile(filePath, reason, packageDir, 0, scanned, results, MAX_DEPTH)
    }
  }
