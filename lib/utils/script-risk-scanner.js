...
// ESM new URL('./file.js', import.meta.url) — common pattern for file-relative paths in ESM modules.
// Captures the path argument so that BFS can follow scripts referenced this way.
const LOCAL_NEW_URL_RE = /\bnew\s+URL\s*\(\s*(['"`])(\.{1,2}\/(?:(?!\1)[^\n)])+)\1\s*,\s*import\.meta\.url\s*\)/g
// require.resolve('./path') — resolves a local module path without loading it.
// Commonly piped into spawn/exec: spawn(process.execPath, [require.resolve('./scripts/build')])
// The resolved path is relative to the calling file, same as require().
const LOCAL_REQUIRE_RESOLVE_RE = /\brequire\.resolve\s*\(\s*(['"`])(\.{1,2}\/(?:(?!\1)[^\n)])+)\1\s*\)/g
...
// Remove `import type ...` and `export type ...` lines from content before
// extracting module references.  These constructs are completely erased by the
// TypeScript compiler — they never appear in emitted JS and create no runtime
// dependency.  Stripping them prevents:
//   • findBareRefs from queueing @types/* and other type-only packages as bare
//     follows, wasting fetch budget on packages with no lifecycle scripts.
//   • findLocalRefs from fetching type-only re-export barrels that carry no
//     executable signals.
// Only lines whose first non-whitespace token is `import type` or `export type`
// are stripped; inline type imports (`import { type X, Y }`) are preserved
// because the line still creates a runtime module binding for Y.
//
// IMPORTANT — ESM binding-name edge case:
//   `import type from './foo'`  is VALID ESM JavaScript where `type` is the
//   default import binding name and `from` is the keyword.  This must NOT be
//   stripped.  The lookahead (?![{*]|(?!from\b)\w) is used to distinguish:
//     • import type { X }  → `{` after type → TypeScript, strip  ✓
//     • import type * as X → `*` after type → TypeScript, strip  ✓
//     • import type Foo    → identifier ≠ 'from' after type → TypeScript, strip ✓
//     • import type from   → next word IS 'from' → ESM binding, preserve ✓
const STRIP_TYPE_ONLY_RE = /^[ \t]*(?:import|export)\s+type\s*(?:[{*]|(?!from\b)[\w$_])[^\n]*/gm
const stripTypeOnlyImports = (content) => content.replace(STRIP_TYPE_ONLY_RE, '')

const findLocalRefs = (content) => {
  const stripped = stripTypeOnlyImports(content)
  const refs = new Set()
  for (const re of [LOCAL_REQUIRE_RE, LOCAL_IMPORT_FROM_RE, LOCAL_IMPORT_BARE_RE, LOCAL_DYNAMIC_IMPORT_RE, LOCAL_IMPORT_META_RESOLVE_RE, LOCAL_NEW_URL_RE, LOCAL_REQUIRE_RESOLVE_RE]) {
    re.lastIndex = 0
    let match
    while ((match = re.exec(stripped)) !== null) {
      refs.add(match[2])
  ...
  let match
  while ((match = SHELL_SOURCE_RE.exec(stripped)) !== null) {
...
const parseSingleCommand = (cmdOrParts, packageDir) => {
  ...
  // than an npm package name (e.g. `./preload.js`, `../lib/helper`, `setup.mts`).
  const isLocalSpecifier = (s) =>
    s.startsWith('./') || s.startsWith('../') || /\.(js|mjs|cjs|ts|mts|cts)$/.test(s)
...
const scanFile = async (filePath, reason, packageDir, depth, scanned, results, maxDepth, maxFiles = Infinity) => {
  ...
  if (scanned.has(filePath)) {
    // File already scanned — append the new triggering reason to the existing
    // result entry rather than adding a duplicate.  Multiple lifecycle hooks
    // (e.g. both `postinstall` and `prepare`) can reference the same script;
    // merging their reasons keeps the results array deduplicated by path while
    // still surfacing all the hooks that triggered the scan.
    const cached = scanned.get(filePath)
    /* istanbul ignore else */
    if (cached && !cached.reason.includes(reason)) {
  ...
  for (const ref of localRefs) {
    ...
    if (resolved) {
      references.add(toPosix(path.relative(packageDir, resolved)))
    }
  }
  ...
  if (depth >= maxDepth && localRefs.length > 0) {
    signals.add('depth-limit-reached')
  }
