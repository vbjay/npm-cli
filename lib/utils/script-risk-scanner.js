...
const SIGNAL_PATTERNS = [
  ...
  ['binary-download',
    ...
    /\bnapi-postinstall\b|\bdownloadBrowsers?\b|\binstallBrowsers|[A-Z][A-Z0-9_]{2,}BINARY_PATH\b/],
  ['runtime-installer',
    // Detects install scripts that run a secondary npm/yarn/pnpm install as a
    ...
    //   • Seen in the 9router/ktrouter/bluerouter malware cluster and in packages
    //     that "self-repair" native binaries by reinstalling them from the network
    //
    // Two forms:
    //   execSync/exec/spawnSync string form:
    //     execSync("npm install pkg --ignore-scripts=false")
    //   spawn/exec array form:
    //     spawn('npm', ['install', ...]) / exec('npm', 'install', ...)
    /(?:execSync|spawnSync|exec|spawn)\s*\(\s*['"`][^'"`\n]*(?:npm|yarn|pnpm)\s+(?:install|add|i\b)|(?:execSync|spawnSync|exec|spawn)\s*\(\s*['"`](?:npm|yarn|pnpm)['"`]\s*,/],
  ['shell-network-fetch',
