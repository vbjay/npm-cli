...
const scanFile = async (filePath, reason, packageDir, depth, scanned, results, maxDepth) => {
  ...
  // external-url is derived from actual non-comment URLs found in code,
  // not from the raw pattern match, so comment-only URLs don't trigger it.
  if (urls.length > 0) {
...
const scanInlineCommand = (event, cmd, results) => {
  ...
  // No comment filtering needed — lifecycle commands are not source code.
  const urls = []
  ...
  if (urls.length > 0 && !signals.includes('external-url')) {
    signals.push('external-url')
  }
  if (signals.length === 0) {
    return
